...
class BiofilmNewtonSolver:
    ...
    def __init__(self, dt=1e-5, maxtimestep=2500, eps=1e-6, Kp1=1e-4,
        ...
        if active_species is not None:
            ...
            # Gracefully fall back to full-state indexing when an empty list or
            # out-of-range indices are provided to avoid accidental NameErrors
            # or ``max()`` failures that obscure the real solver wiring issues.
            if len(active_species) == 0 or max(active_species) >= self.n:
